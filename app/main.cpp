...
int main()
{
    ...
    float avg_price = .0;
    const auto duration = measure_execution_time<ns>(
        [&company, &avg_price]() { avg_price = company.calculate_average_price(); });
    std::cout << "Average price: " << avg_price << " (execution time: " << duration.count() << "ns)"
              << std::endl;
}
